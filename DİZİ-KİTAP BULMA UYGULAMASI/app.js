...
let kitap1={
    isim:"Her şeyi Düşünme",    //kitapları obje olarak tanımladık.
    yazar:"Anne Bogel",
    fiyat:25,
    raf:"1.5 RAF"
}
...
let raflar=[
    ...
    [raf41,raf42,raf43,raf44,raf45],   //!objeler arasına virgül koymazsan hata verir!!
...
function rafOlustur(){
    console.clear();//ilk başta konsol temizlensin.
    let satir="";
   //for döngüsünü method içinde tanımlıyouz,daha sonra da kullanabilelim diye.

    for(let i=0;i<raflar.length;i++){    
        for(let j=0;j<5;j++){
            satir+="|"+(raflar[i][j].goster ? raflar[i][j].kod:"---")+"";
        }
        //döngüden çkınca bir alt satıra geç
        console.log(satir);
        console.log("----------------");
        satir="";
    }
...
if(rafKod!=null){
    ...
    alert("girdiğiniz kitap kütüphanemizde yoktur.");
}
